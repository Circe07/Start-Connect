...
export default function LoginScreen({ navigation }: any) {
  ...
  const handleLogin = async () => {
    ...
    if (!passwordValidation.isValid) {
      Alert.alert('Invalid Password', passwordValidation.error || 'Password must be at least 6 characters long');
      return;
    }
    ...
    try {
      ...
      if (!loginResult.success) {
        ...
        setIsLoading(false);
        Alert.alert('Login Failed', loginResult.error || 'Invalid email or password. Please try again.', [
          { text: 'OK' }
        ]);
        return;
      }
      ...
      console.log('✅ Token verified and stored (length:', storedToken.length, ')');
      ...
      try {
        ...
        if (userResult.success) {
          if (userResult.user) {
            ...
            const hasUserFields = responseData.id || responseData.email || responseData.username || responseData.name || responseData.uid;
            if (hasUserFields) {
              // Extract user fields from response (remove metadata like success, status, etc.)
              const { success, error, message, status, ...userFields } = responseData;
              if (Object.keys(userFields).length > 0) {
                userData = userFields;
                console.log('✅ User data extracted from response (removed metadata)');
              }
            } else {
              console.error('❌ getCurrentUser() succeeded but no user data found');
              console.error('❌ Response keys:', Object.keys(userResult));
              console.error('❌ Full response:', JSON.stringify(userResult, null, 2));
            }
          }
        } else {
          ...
          console.error('❌ Full error response:', JSON.stringify(userResult, null, 2));
      ...
      if (!userData) {
        ...
        console.error('❌ Login was successful, token is stored, but user profile is missing');
        console.error('❌ This should not happen - user data must be available');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsLoading(false);
        Alert.alert('Google Login', 'Google Sign-In requires configuration. For now, please use the regular Sign In button.', [
          { text: 'OK' }
        ]);
      }, 400);
    } catch (error) {
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsLoading(false);
        Alert.alert('Apple Login', 'Apple Sign-In requires configuration. For now, please use the regular Sign In button.', [
          { text: 'OK' }
        ]);
      }, 400);
    } catch (e) {
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: isDarkMode ? '#000' : '#fff' }]}> 
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: isDarkMode ? '#bdbdbd' : '#9E9E9E' }]}>Welcome back</Text>
        </View>
        ...
        <View style={styles.form}>
          ...
          <Pressable style={({ pressed }) => [styles.forgotPassword, { opacity: pressed ? 0.7 : 1 }]}>
            <Text style={[styles.forgotPasswordText, { color: BRAND_ORANGE }]}>Forgot Password?</Text>
          </Pressable>

          <Pressable 
            style={({ pressed }) => [
              styles.loginButton, 
              { opacity: pressed || isLoading ? 0.85 : 1, backgroundColor: isLoading ? '#ccc' : BRAND_ORANGE }
            ]} 
            onPress={handleLogin}
            disabled={isLoading}
          >
          ...
          <View style={styles.divider}>
            <View style={[styles.dividerLine, { backgroundColor: isDarkMode ? '#333' : '#e0e0e0' }]} />
            <Text style={[styles.dividerText, { color: isDarkMode ? '#666' : '#999' }]}>OR</Text>
            <View style={[styles.dividerLine, { backgroundColor: isDarkMode ? '#333' : '#e0e0e0' }]} />
          </View>
          ...
          >
            <Image 
              ...
              resizeMode="contain"
            />
            <Text style={[styles.googleButtonText, { color: isDarkMode ? '#f2f2f2' : '#000' }]}>
          ...
          >
            <Image
                ...
                resizeMode="contain"
            />
            <Text style={[styles.appleButtonText, { color: isDarkMode ? '#f2f2f2' : '#000' }]}>
              Continue with Apple
            </Text>
          </Pressable>
        </View>
        ...
        <View style={styles.footer}>
          <Text style={[styles.footerText, { color: isDarkMode ? '#9E9E9E' : '#BDBDBD' }]}>
            ...
            <Pressable onPress={() => navigation.navigate('SignUp')}>
              <Text style={[styles.footerLink, { color: BRAND_ORANGE }]}>Sign up</Text>
            </Pressable>
          </Text>
        </View>
      </View>
