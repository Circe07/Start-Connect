...
exports.getMyGroups = async (req, res) => {
  try {
    ...
    const snapshot = await groupsRef().where('memberIds', 'array-contains', userId).get();
    const groups = snapshot.docs.map((doc) => Group.fromFirestore(doc));
...
exports.joinGroup = async (req, res) => {
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (group.members.some((m) => m.userId === userId)) throw new Error('already_member');

      if (group.members.length >= (group.maxMembers || 10)) throw new Error('full');
...
exports.leaveGroup = async (req, res) => {
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (!group.members.some((m) => m.userId === userId)) throw new Error('not_member');
      ...
      if (group.userId === userId) throw new Error('owner_must_transfer');
...
exports.newPost = async (req, res) => {
  try {
    ...
    if (!groupDoc.exists) return res.status(404).json({ message: 'El grupo no existe' });
...
exports.transferOwner = async (req, res) => {
  try {
    ...
    if (group.userId !== userId) return res.status(403).json({ message: 'No eres el propietario' });

    if (!group.members.some((m) => m.userId === newOwnerId))
      return res.status(400).json({ message: 'Debe ser miembro' });

    if (newOwnerId === userId) return res.status(400).json({ message: 'Ya eres propietario' });
...
exports.updateGroup = async (req, res) => {
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (group.userId !== userId) throw new Error('unauthorized');
...
exports.deleteGroup = async (req, res) => {
  try {
    ...
    if (doc.data().userId !== userId) return res.status(403).json({ message: 'No tienes permiso' });
...
exports.deletePost = async (req, res) => {
  try {
    ...
    if (!postDoc.exists) return res.status(404).json({ message: 'La publicación no existe.' });
...
exports.getMessages = async (req, res) => {
  try {
    ...
    let query = groupRef.collection('messages').orderBy('createdAt', 'desc').limit(limit);
...
exports.deleteMessage = async (req, res) => {
  try {
    ...
    const [groupDoc, messageDoc] = await Promise.all([groupRef.get(), messageRef.get()]);
