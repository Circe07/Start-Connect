...
const fetchUserPendingGroupIds = async (userId) => {
  ...
  const snapshot = await groupRequestsRef().where("userId", "==", userId).get();

  return new Set(
    snapshot.docs.map((doc) => doc.data()?.groupId).filter(Boolean)
  );
};

const decorateGroupWithViewerState = (
  group,
  userId,
...
exports.getPublicGroups = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      nextStartAfterId:
        snapshot.size === limit ? snapshot.docs.at(-1).id : null,
    });
  } catch (error) {
...
exports.joinGroup = async (req, res) => {
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (!group.isPublic) throw new Error("requires_request");
...
exports.removeMember = async (req, res) => {
  try {
    ...
    if (group.userId !== userId)
      return res
        .status(403)
        .json({ message: "Solo el propietario puede eliminar" });
...
exports.addComment = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Comentario añadido.", commentId: commentRef.id });
  } catch (error) {
